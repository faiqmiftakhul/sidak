...
import { BungkusTabel, Kepala, Pill } from '../components/Common'
import Ikon from '../components/Ikon'
import { INFO, KOTA, KabModul, MODUL, Modul, judulKab, num, oe, pct, rp, useData } from '../lib/data'
import { usePalet } from '../lib/tema'

const NAMA_TINGKAT: Record<Tingkat, string> = {
  L1: 'L1 · Jawa Tengah', L2: 'L2 · Kecamatan Semarang', L3: 'L3 · Aliran pasien',
}
...
export default function Peta() {
  ...
  return (
    <>
      <Kepala
        eyebrow="Peta"
        judul="Dari provinsi ke kecamatan dalam dua klik"
        sub="L1: hasil deteksi per kabupaten/kota Jawa Tengah dari Data Sampel. L2: konteks Kota Semarang dari data publik (penduduk, faskes nyata). L3: aliran pasien dari kabupaten asal ke RS di Semarang."
      />

      <div className="toolbar" role="group" aria-label="Pengaturan peta">
        {(['L1', 'L2', 'L3'] as Tingkat[]).map(t => (
          <button key={t} type="button" className={'chip' + (tingkat === t ? ' on' : '')} aria-pressed={tingkat === t} onClick={() => setTingkat(t)}>{NAMA_TINGKAT[t]}</button>
        ))}
        <span className="pisah" />
        {tingkat === 'L1' && <>
          <select value={m} onChange={e => setM(e.target.value as Modul)} aria-label="Pilih modul">{MODUL.map(k => <option key={k} value={k}>#{INFO[k].nomor} {INFO[k].nama}</option>)}</select>
          <select value={indL1} onChange={e => setIndL1(e.target.value as IndikatorL1)} aria-label="Pilih indikator">{(Object.keys(LABEL_L1) as IndikatorL1[]).map(k => <option key={k} value={k}>{LABEL_L1[k]}</option>)}</select>
        </>}
        {tingkat === 'L2' && <>
          <select value={indL2} onChange={e => setIndL2(e.target.value as IndikatorL2)} aria-label="Pilih indikator demografi">{(Object.keys(LABEL_L2) as IndikatorL2[]).map(k => <option key={k} value={k}>{LABEL_L2[k]}</option>)}</select>
          <label className="saklar"><input type="checkbox" checked={fas} onChange={e => setFas(e.target.checked)} /> Titik faskes (OSM)</label>
        </>}
        {tingkat === 'L3' && <select value={alir} onChange={e => setAlir(e.target.value as typeof alir)} aria-label="Pilih jenis aliran"><option value="readmisi">Readmisi: asal domisili peserta</option><option value="rujukan">Rujukan FKTP: asal FKTP terdaftar</option></select>}
      </div>

      {tingkat === 'L2' && <div className="notice" style={{ marginBottom: 'var(--s3)' }}>Titik faskes dan demografi di lapisan ini berasal dari data publik (OpenStreetMap, BPS) dan <b>bukan</b> hasil deteksi. Hasil deteksi pada Data Sampel bersifat anonim per faskes dan ditampilkan di panel kanan, bukan di titik peta.</div>}
        ...
        <div className="map-info">
          {tingkat === 'L2' ? (
            ...
            {pilihKab.d?.[m] ? (() => { const d = pilihKab.d![m]!; return <div className="kv" style={{ marginTop: 6, fontSize: 12 }}>
              ...
              <span className="k">Perlu perhatian</span><span style={{ color: palet.status.perhatian.warna, fontWeight: 600 }}>{d.n_perhatian}</span>
              <span className="k">Diamati</span><span style={{ color: palet.status.diamati.warna, fontWeight: 600 }}>{d.n_diamati}</span>
            ...
            {fkKab.length > 0 && <>
              <div className="eyebrow" style={{ marginTop: 12 }}>Faskes (urut O/E)</div>
              <BungkusTabel>
            ...
            {pilihKab.kab === KOTA && <div style={{ marginTop: 10 }}><button type="button" className="chip" onClick={() => setTingkat('L2')}>Buka kecamatan Semarang <Ikon nama="panah" ukuran={14} /></button></div>}
