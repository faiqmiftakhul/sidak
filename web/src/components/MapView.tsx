...
const FALLBACK_SEQ = ['#FBF3E2', '#F7E3C0', '#F2C97A', '#E2A13D', '#B45309']
const FALLBACK_OE = ['#0F9D8F', '#A16207', '#C2410C', '#7C2D12']
...
function seqRamp(): string[] { return FALLBACK_SEQ.map((_, i) => cv(`--ramp-${i + 1}`, FALLBACK_SEQ[i])) }
function oeRamp(): string[] { return FALLBACK_OE.map((_, i) => cv(`--oe-${i + 1}`, FALLBACK_OE[i])) }
...
export default function MapView({ tingkat, modul, indL1 = 'OE', indL2 = 'penduduk_2025', tampilFaskes = true, aliranModul = 'readmisi', onPilihKab, onPilihKec, kecil }: Props) {
  ...
  useEffect(() => {
    ...
    const m = new maplibregl.Map({
      ...
      style: { version: 8, sources: {}, layers: [{ id: 'bg', type: 'background', paint: { 'background-color': '#E9EEF4' } }] },
    ...
    m.on('load', () => {
      ...
      m.addLayer({ id: 'kab-sel', type: 'line', source: 'kab', paint: { 'line-color': '#0E7490', 'line-width': 2.5 }, filter: ['==', ['get', 'kab'], KOTA] })
      ...
      m.addLayer({ id: 'fas-rs', type: 'circle', source: 'fas', layout: { visibility: 'none' }, filter: ['==', ['get', 'jenis'], 'hospital'], paint: { 'circle-radius': 6, 'circle-color': '#0E7490', 'circle-stroke-color': '#fff', 'circle-stroke-width': 1.5 } })
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      rows.forEach(r => {
        ...
        const warna = aliranModul === 'readmisi' ? warnaOE(oeV) : '#0E7490'
  ...
  const legend = tingkat === 'L2'
    ? <div className="map-legend"><b>{LABEL_L2[indL2]}</b> (kuintil) <br />{seqRamp().map((c, i) => <span key={i} className="sw" style={{ background: c }} />)} rendah → tinggi<br />
      {tampilFaskes && <><span className="sw" style={{ background: cv('--brand', '#0E7490'), borderRadius: 7 }} />RS &nbsp;<span className="sw" style={{ background: cv('--green', '#148F63'), borderRadius: 7 }} />klinik/praktik (OSM)</>}</div>
    : indL1 === 'OE' || tingkat === 'L3'
      ? <div className="map-legend"><b>{tingkat === 'L3' && aliranModul === 'rujukan' ? 'Aliran rujukan (tebal = jumlah)' : 'Rasio O/E'}</b><br />
        {tingkat === 'L3' && aliranModul === 'rujukan' ? null : (() => { const oe = oeRamp(); return <>
          <span className="sw" style={{ background: oe[0] }} />&lt; 0,90 &nbsp;<span className="sw" style={{ background: oe[1] }} />0,90–1,05<br />
          <span className="sw" style={{ background: oe[2] }} />1,05–1,20 &nbsp;<span className="sw" style={{ background: oe[3] }} />&gt; 1,20</> })()}</div>
      : <div className="map-legend"><b>{LABEL_L1[indL1]}</b> (kuintil)<br />{seqRamp().map((c, i) => <span key={i} className="sw" style={{ background: c }} />)} rendah → tinggi</div>
